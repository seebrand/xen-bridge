...
def index(): 
    url = None
    server = 3 #request.forms.get("server")
...
def home(username):
    if len(username)<2:
        username = None
    # deals = read_deals()
    # return template('home.html', deals=deals, play=False)
    deals = read_deals(username,0)
    return template('home.html', deals=deals, play=False,username=username)    

@app.route('/play/<username>')
def home(username):
    deals = read_deals(username,0)
    # return template('home.tpl', deals=deals, play=True)
    return template('home.html', deals=deals, play=True,username=username)
...
def delete_deal(deal_id):
    ...
    try:
        ...
        if referrer:
            ...
            return redirect('/home/xen')  # Default fallback
    except KeyError:
        print("Deal not found")
        raise HTTPError(404, "Deal not found")
