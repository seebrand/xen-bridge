...
<body>
    <div>
        <h1>API for XEN. Version 0.8.7.4</h1>
    </div>
    <div id="loader"></div> 
    <div id="dealdiv">
        ...
        <small id="tooltip2" style="display: none;">Please enter all the played cards including cards from XEN</small><br><br>

                    
        <button onclick="callAPI('bid')">Ask for bid</button>&nbsp;&nbsp;
        <button onclick="callAPI('play')">Ask for play</button><br><br>
        <button onclick="callAPI('lead')">Ask for lead</button><br><br>
        <button onclick="callAPI('claim')">Claim</button><br><br>
        <button onclick="callAPI('contract')">Ask for contract</button>&nbsp;&nbsp;
    ...
    <script>
                ...
                for (const candidate of data.candidates) {
                    ...
                    if ("msg" in candidate) {
                        if (candidate['msg'].length > 0) {
                            ...
                            html += "<td onmouseover=XENShowPopup(this,'" + escapedMsg + "',0) onmouseout=XENHidePopup()>[calculations]</td>"
                        }
                    }
                    html += '</tr>'
                }
        ...
        async function callAPI(action) {
            ...
            try {           
                ...
                if (action == 'bid') {
                    if (data.alert == "True") {
                        document.getElementById('result').innerText = `XEN Suggest: ${data.bid}*`;
                    } else {
                        document.getElementById('result').innerText = `XEN Suggest: ${data.bid}`;
                    }
                    displayBid(data);
                ...
                if (action == 'contract') {
                    document.getElementById('result').innerText = 'XEN Suggest:';
                    for (var key in data) {
                ...
                if ((action == 'play') || (action == 'lead')) {
                    ...
                    document.getElementById('result').innerText = `XEN Suggest: ${data.card}`;
        ...
        function XENShowPopup(obj, text, pos) {
            var div = document.getElementById("XENPopup");
            text = text.replace(/\&/g,"&amp;");
            text = text.replace(/\</g,"&lt;");
        ...
        function XENHidePopup() {
            var div = document.getElementById("XENPopup");
            div.style.visibility = "hidden";
        }
    </script>
    <div id=XENPopup style="position: absolute; color: black; background-color: #ffffe1; border: thin solid black; text-align: left; padding-left: 4px; padding-right: 4px; font: small 'Arial', sans-serif; visibility: hidden"></div>
</body>
