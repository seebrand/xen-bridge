    ...
    def nextplay(self, player_i, shown_out_suits, missing_cards):
        ...
        if self.models.use_real_imp_or_mp:
            ...
            for key in card_ev.keys():
                card_result[key] = (round(e_tricks[key], 3), round(card_ev[key],3), round(making[key],3), msg)
                if self.verbose:
                    print(f'{Card.from_code(key)} {round(e_tricks[key],3):0.3f} {round(card_ev[key],2):5.3f} {round(making[key],3):0.3f}')
